 ...
 * if (result.success) {
 *   logger.info(`Tweet posted: ${result.tweetId}`);
 * }
 * ```
 */
export class XService {
  ...
  async fetchTweetMetrics(tweetIds: string[]): Promise<
    Array<{
      tweetId: string;
      capturedAt: string;
      metrics: {
  ...
  > {
    ...
    } as Parameters<typeof this.client.v2.tweets>[1]);
    ...
    return (response.data || []).map((tweet) => ({
      ...
      metrics: {
        impressions:
          typeof tweet.public_metrics?.impression_count === "number"
        ...
        shares:
          ...
          typeof tweet.public_metrics?.quote_count === "number"

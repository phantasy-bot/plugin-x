...
async function handleTweetApproval(
  ...
  try {
    ...
    if (
      notification.externalIds?.partyHQTweetId &&
      partyHQSyncService.isConfigured()
    ) {
      const syncResult = await partyHQSyncService.notifyApproval({
        tweetId: notification.externalIds.partyHQTweetId,
      ...
      if (!syncResult.success) {
        log.warn("Failed to sync tweet approval to Party-HQ", {
          notificationId: notification.id,
...
async function handleReplyApproval(
  ...
  try {
    ...
    if (
      ...
      if (!syncResult.success) {
        log.warn("Failed to sync reply approval to Party-HQ", {
          notificationId: notification.id,
          error: syncResult.error,
        });
      }

...
__export(index_exports, {
  ...
  XPlugin: () => XPlugin,
  XService: () => XService,
  approveXNotification: () => approveXNotification,
  default: () => x_plugin_default,
  defaultTwitterAdvancedConfig: () => defaultTwitterAdvancedConfig,
  getXRateLimitStatus: () => getXRateLimitStatus,
  isXApprovalNotificationType: () => isXApprovalNotificationType,
  xService: () => xService
});
...
var logger = (0, import_plugin_runtime.createPluginModuleLogger)("XService");
var XService = class _XService {
  ...
  async hasCredentials() {
    try {
      const platformConfig = await import_plugin_runtime.PlatformConfigService.getInstance().get("twitter");
      if (!platformConfig) {
        return false;
      }
  ...
  async getClient() {
    ...
    this.client = new import_twitter_api_v2.TwitterApi({
      ...
      if (options?.mediaUrls && options.mediaUrls.length > 0) {
        for (const mediaUrl of options.mediaUrls) {
          try {
            ...
            const mediaResponse = await (0, import_plugin_runtime.fetchWithTimeout)(mediaUrl, {
              timeout: 3e4
            });
            if (!mediaResponse.ok) {
  ...
  async createDraftTweet(text) {
    try {
      ...
      const notificationId = (0, import_plugin_runtime.createRuntimeId)("tweet_draft");
      const notification = {
        id: notificationId,
        type: "tweet_approval",
        platform: "twitter",
  ...
  async createDraftReply(tweetId, text) {
    try {
      ...
      const notificationId = (0, import_plugin_runtime.createRuntimeId)("reply_draft");
      const notification = {
        id: notificationId,
        type: "reply_approval",
        platform: "twitter",
  ...
  async storeNotification(notification) {
    try {
      const existingData = await import_plugin_runtime.kvService.get("notifications") || {
        notifications: []
      };
      ...
      await import_plugin_runtime.kvService.set("notifications", existingData);
...
// src/x-plugin.ts
var log = (0, import_plugin_runtime2.createPluginModuleLogger)("XPlugin");
var XPlugin = class extends import_plugins.BasePlugin {
  name = "x-plugin";
  version = "0.1.0-beta";
...
var import_plugin_runtime3 = require("@phantasy/agent/plugin-runtime");
var log2 = (0, import_plugin_runtime3.createPluginModuleLogger)("XNotificationApprovals");
...
var import_plugin_runtime4 = require("@phantasy/agent/plugin-runtime");
var logger2 = (0, import_plugin_runtime4.createPluginModuleLogger)("TwitterRoutes");
var TwitterRoutes = class {
  async getPluginManager(context) {
    return (0, import_plugin_runtime4.getRuntimePluginManager)({ env: context.env });
  }
...
var import_twitter_api_v22 = require("twitter-api-v2");
var import_plugin_runtime5 = require("@phantasy/agent/plugin-runtime");
...
// src/twitter-integration.ts
var logger3 = (0, import_plugin_runtime5.createPluginModuleLogger)("TwitterIntegration");
...
var TwitterIntegration = class {
  ...
  constructor(env) {
    this.env = env;
    this.logStorage = import_plugin_runtime5.LogStorage.getInstance();
  }
  static async test(agent) {
    try {
      ...
      const client = new import_twitter_api_v22.TwitterApi({
  ...
  getClient(config) {
    ...
    this.cachedClient = new import_twitter_api_v22.TwitterApi({
      appKey: config.apiKey,
      appSecret: config.apiSecret,
      accessToken: config.accessToken,
      accessSecret: config.accessSecret
